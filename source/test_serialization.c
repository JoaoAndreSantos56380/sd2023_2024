...
void test_keyArray_to_buffer() {
    ...
    char** good_keys_array = (char**)malloc(num_strings * sizeof(char*)); // alocar memória para o array
    ...
    for (int i = 0; i < nkeys; i++) {
        assert(strcmp(good_keys_buf_ptr, good_keys_array[i]) == 0);
        good_keys_buf_ptr += strlen(good_keys_buf_ptr) + 1;
    }

    char** bad_keys_buf = malloc(sizeof(int));
    assert(keyArray_to_buffer(good_keys_array, bad_keys_buf) == -1);
...
void test_buffer_to_keyArray() {
    ...
    char** good_keys_buf;
    keyArray_to_buffer(good_keys_array, bad_keys_buf);

    char** result = buffer_to_keyArray(good_keys_buf);
    for(int i = 0; i < num_keys; i++) {
        assert(strcmp(result[i], good_keys_array[i]) == 0);
    }
