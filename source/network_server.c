...
int network_server_init(short port) {
	...
	// Ignorar SIGPIPE signal para o server não crashar se a socket fechar unexpectedly
...
int network_send(int client_socket, struct message_t* msg) {
	...
	message_t__free_unpacked(msg, NULL);  // Free da message depois de enviar
...
int network_server_close(int socket) {
	server_running = 0;	// Signal server e as threads para fechar

	close(socket);	 // Unblock à listening socket

	// Unblock às client sockets
	...
	// Esperar pelas threads todas fecharem
	for (int i = 1; i < next_thread_number; i++) {
		int num_of_chars = (int)(ceil(log10(i)) + 1);
		char key[7 + num_of_chars];
		sprintf(key, "thread_%d", i);
