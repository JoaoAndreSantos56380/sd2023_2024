...
int test_data_destroy() {
    ...
    int result;
    struct data_t* good_data = data_create(15, "good");
    struct data_t* bad_data; // esta é má porque não alocamos memória!
...
int test_data_dup() {
    ...
    int result;
    struct data_t* good_data = data_create(27, "good");
    struct data_t* bad_data; // esta é má porque não alocamos memória!
    ...
    result += (data_replace(dup_good_data, 10, "good dup") == 0);
    result += (data_dup(dup_good_data) != NULL);
...
int test_data_replace() {
    ...
    int result;
    struct data_t* good_data = data_create(50, "good");
    struct data_t* bad_data; // esta é má porque não alocamos memória!
    ...
    result += (strcmp(good_data->data, "evil")); //se os dois estiverem bem então duplicou bem.
